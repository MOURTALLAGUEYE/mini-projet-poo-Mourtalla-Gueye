...
def test_emprunt_rend_le_document_indisponible():
    media, livre, awa = _mediatheque_avec_livre()
...
def test_emprunter_document_deja_prete_leve_exception():
    media, livre, awa = _mediatheque_avec_livre()
    media.emprunter(awa.numero, "L001")
...
def test_quatrieme_emprunt_leve_trop_d_emprunts():
    ...
    for i in range(4):
        media.ajouter_document(
            Livre(f"Livre {i}", 2020, f"L00{i}", auteur="X", nb_pages=100)
        )
    for i in range(3):
